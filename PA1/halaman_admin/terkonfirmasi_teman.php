...
<body>
    ...
    <main class="mt-5 pt-3">
       <div class="container-fluid">
          <div class="row">
            <div class="col-md-12 fw-bold fs-3 text-center text-light">Data Pesanan Terkonfirmasi Produk Sambal Teri Andaliman Mauas JKM</div> 
          </div>
  ...
  <tbody>
    <?php 
    require '../config.php';
    $data = mysqli_query($conn,"SELECT * FROM checkout_produk INNER JOIN akun ON checkout_produk.akun_id = akun.id_akun WHERE nama_produk = 3 AND status_bayar = 'Terkonfirmasi'");
    $no = 1;
    foreach($data as $cookies) {
    ?>
    <tr>
      <th scope="row"><?php echo $no++?></th>
      <td><?php echo $cookies['nama_lengkap'];?></td>
      <td><?php echo $cookies['quantity'];?></td>

      <td><?php echo $cookies['tanggal_pemesanan']?></td>
    </tr>
    <?php }?>
  </tbody>
</table>
